...
public class TestDFSAdmin {
  ...
  @Test(timeout = 120000)
  public void testPrintTopologyWithState() throws Exception {
    redirectStream();
    final Configuration dfsConf = new HdfsConfiguration();
    final File baseDir = new File(
            PathUtils.getTestDir(getClass()),
    ...
    try (MiniDFSCluster miniCluster = new MiniDFSCluster.Builder(dfsConf)
      ...
      /* verify results */
      assertEquals(0, ret);
      assertThat(outs.get(1),
              is(allOf(containsString(DatanodeInfo.AdminStates.NORMAL.toString()))));
      assertThat(outs.get(4),
              is(allOf(containsString(DatanodeInfo.AdminStates.IN_MAINTENANCE.toString()))));
      assertThat(outs.get(7),
              is(allOf(containsString(DatanodeInfo.AdminStates.DECOMMISSIONED.toString()))));
      assertThat(outs.get(10),
              is(allOf(containsString(DatanodeInfo.AdminStates.NORMAL.toString()))));
    }
  }
